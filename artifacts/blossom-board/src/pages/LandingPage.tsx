...
export default function LandingPage({ onNavigate }: LandingPageProps) {
  ...
  return (
    <div style={{ paddingTop: 60 }}>
      ...
      {/* WHAT DO YOU NEED TO BLOSSOM - New hexagonal/masonry layout */}
      <section style={{ background: 'linear-gradient(135deg, #FCE4EC 0%, #FFF0F5 50%, #FFDDE9 100%)', padding: '5rem 2rem' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', textAlign: 'center' }}>
          <div style={{ fontSize: '0.8rem', fontWeight: 800, letterSpacing: '0.12em', color: '#E91E8C', textTransform: 'uppercase', marginBottom: '0.6rem' }}>✦ Your creative essentials</div>
          <h2 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', fontWeight: 900, color: '#3D1A2E', marginBottom: '0.5rem' }}>What do you need to blossom? 🌸</h2>
          <p style={{ color: '#AD6590', fontSize: '1rem', marginBottom: '3.5rem' }}>Everything your creative mind craves, beautifully wrapped in pink.</p>

          {/* Masonry-style asymmetric grid */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.4fr 1fr', gridTemplateRows: 'auto auto', gap: '1.2rem', alignItems: 'start' }}>
            {/* Left column */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
              {BLOSSOM_ITEMS.slice(0, 2).map((item, i) => (
                <motion.div key={item.label}
                  initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }}
                  viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                  style={{
                    background: 'white', borderRadius: 20, padding: '1.5rem',
                    border: '1.5px solid #FCE4EC', display: 'flex', alignItems: 'center', gap: '1rem',
                    boxShadow: '0 4px 20px rgba(233,30,140,0.08)',
                  }}>
                  <div style={{ width: 52, height: 52, borderRadius: '50%', background: item.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem', flexShrink: 0 }}>{item.emoji}</div>
                  <div style={{ fontSize: '0.9rem', fontWeight: 700, color: '#3D1A2E', textAlign: 'left' }}>{item.label}</div>
                </motion.div>
              ))}
            </div>
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
              style={{
                ...
                color: 'white', textAlign: 'center', gridRow: '1 / 3',
                display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.2rem',
              }}>
              <div style={{ fontSize: '4rem' }}>🌸</div>
              <div style={{ fontSize: '1.6rem', fontWeight: 900, lineHeight: 1.2 }}>A canvas where ideas bloom</div>
              <div style={{ fontSize: '0.95rem', opacity: 0.9, lineHeight: 1.6 }}>Freehand drawing, sticky notes, shapes, and collaboration — all in one place. Everything you need to make your ideas flourish.</div>
              <button onClick={() => onNavigate('board')} style={{
                background: 'white', color: '#E91E8C',
                padding: '0.7rem 1.8rem', borderRadius: 50,
                border: 'none', cursor: 'pointer',
                fontFamily: 'Nunito, sans-serif', fontWeight: 800, fontSize: '0.95rem',
                marginTop: '0.5rem', transition: 'transform 0.2s',
              }}
            ...
            {/* Right column */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
              {BLOSSOM_ITEMS.slice(2, 4).map((item, i) => (
                <motion.div key={item.label}
                  initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }}
                  viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                  style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.2rem', marginTop: '1.2rem' }}>
            {BLOSSOM_ITEMS.slice(4).map((item, i) => (
              <motion.div key={item.label}
                initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                style={{
                  background: 'white', borderRadius: 20, padding: '1.4rem 1.8rem',
                  border: '1.5px solid #FCE4EC', display: 'flex', alignItems: 'center', gap: '1rem',
                  boxShadow: '0 4px 20px rgba(233,30,140,0.08)',
                }}>
                <div style={{ width: 48, height: 48, borderRadius: '50%', background: item.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem', flexShrink: 0 }}>{item.emoji}</div>
                <div style={{ fontSize: '0.9rem', fontWeight: 700, color: '#3D1A2E' }}>{item.label}</div>
              </motion.div>
            ))}
          </div>
        </div>
